  ...
  describe '配送先情報の保存' do
    ...
    context '配送先情報の保存が出来ないとき' do
      ...
      it "都道府の「---」が選択されている状態では保存ができない" do
        @order_shipping_id = 1
        @item.valid?
        expect( @order_shipping.errors.full_messages).to include("can't be blank")
      end
      ...
      it '番地が空では保存できない' do
        ...
        @order_shipping.valid?
        expect(@order_shipping.errors.full_messages).to include("Street_address can't be blank")
      end
